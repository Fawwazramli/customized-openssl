...
#define SAIFERV12_ADD_ROUND_KEY(RK, T0, T1, T2, T3)  \
    do {                                        \
        (T0) ^= (RK)->u[0];                     \
        (T1) ^= (RK)->u[1];                     \
        (T2) ^= (RK)->u[2];                     \
...
#define SAIFERV12_SBOX_LAYER1_WITH_PRE_DIFF(T0, T1, T2, T3)  \
    do {                                                \
        (T0) =                                          \
            S1[GET_U8_BE(T0, 0)] ^                      \
            S2[GET_U8_BE(T0, 1)] ^                      \
...
#define SAIFERV12_SBOX_LAYER2_WITH_PRE_DIFF(T0, T1, T2, T3)  \
    do {                                                \
        (T0) =                                          \
            X1[GET_U8_BE(T0, 0)] ^                      \
            X2[GET_U8_BE(T0, 1)] ^                      \
...
#define SAIFERV12_DIFF_WORD(T0,T1,T2,T3) \
    do {                            \
        (T1) ^= (T2);               \
        (T2) ^= (T3);               \
        (T0) ^= (T1);               \
...
#define SAIFERV12_DIFF_BYTE(T0, T1, T2, T3)                                  \
    do {                                                                \
...
#define SAIFERV12_SUBST_DIFF_ODD(T0, T1, T2, T3)             \
    do {                                                \
        SAIFERV12_SBOX_LAYER1_WITH_PRE_DIFF(T0, T1, T2, T3); \
        SAIFERV12_DIFF_WORD(T0, T1, T2, T3);                 \
        SAIFERV12_DIFF_BYTE(T0, T1, T2, T3);                 \
        SAIFERV12_DIFF_WORD(T0, T1, T2, T3);                 \
    } while(0)


#define SAIFERV12_SUBST_DIFF_EVEN(T0, T1, T2, T3)            \
    do {                                                \
        SAIFERV12_SBOX_LAYER2_WITH_PRE_DIFF(T0, T1, T2, T3); \
        SAIFERV12_DIFF_WORD(T0, T1, T2, T3);                 \
        SAIFERV12_DIFF_BYTE(T2, T3, T0, T1);                 \
        SAIFERV12_DIFF_WORD(T0, T1, T2, T3);                 \
    } while(0)


#define _SAIFERV12_GSRK(RK, X, Y, Q, R)                  \
    do {                                            \
        (RK)->u[0] =                                \
            ((X)[0]) ^                              \
            (((Y)[((Q)    ) % 4]) >> (R)) ^         \
...
#define SAIFERV12_GSRK(RK, X, Y, N) _SAIFERV12_GSRK(RK, X, Y, 4 - ((N) / 32), (N) % 32)

#define SAIFERV12_DEC_DIFF_BYTE(X, Y, TMP, TMP2)         \
    do {                                            \
        (TMP) = (X);                                \
        (TMP2) = rotr32((TMP), 8);                  \
                  ...
                  const SAIFERV12_KEY *key)
{
    ...
    SAIFERV12_ADD_ROUND_KEY(rk, reg0, reg1, reg2, reg3);
    rk++;

    SAIFERV12_SUBST_DIFF_ODD(reg0, reg1, reg2, reg3);
    SAIFERV12_ADD_ROUND_KEY(rk, reg0, reg1, reg2, reg3);
    rk++;

    while(Nr -= 2){
        SAIFERV12_SUBST_DIFF_EVEN(reg0, reg1, reg2, reg3);
        SAIFERV12_ADD_ROUND_KEY(rk, reg0, reg1, reg2, reg3);
        rk++;
                         ...
                         SAIFERV12_KEY *key)
{
    ...
    SAIFERV12_GSRK(rk, w0, w1, 19);
    rk++;
    SAIFERV12_GSRK(rk, w1, w2, 19);
    rk++;
    SAIFERV12_GSRK(rk, w2, w3, 19);
    rk++;
    SAIFERV12_GSRK(rk, w3, w0, 19);

    rk++;
    SAIFERV12_GSRK(rk, w0, w1, 31);
    rk++;
    SAIFERV12_GSRK(rk, w1, w2, 31);
    rk++;
    SAIFERV12_GSRK(rk, w2, w3, 31);
    rk++;
    SAIFERV12_GSRK(rk, w3, w0, 31);

    rk++;
    SAIFERV12_GSRK(rk, w0, w1, 67);
    rk++;
    SAIFERV12_GSRK(rk, w1, w2, 67);
    rk++;
    SAIFERV12_GSRK(rk, w2, w3, 67);
    rk++;
    SAIFERV12_GSRK(rk, w3, w0, 67);

    rk++;
    SAIFERV12_GSRK(rk, w0, w1, 97);
    if (bits > 128) {
        rk++;
        SAIFERV12_GSRK(rk, w1, w2, 97);
        rk++;
        SAIFERV12_GSRK(rk, w2, w3, 97);
    }
    if (bits > 192) {
        rk++;
        SAIFERV12_GSRK(rk, w3, w0, 97);

        rk++;
        SAIFERV12_GSRK(rk, w0, w1, 109);
    }
...
int saiferv12_set_decrypt_key(const unsigned char *userKey, const int bits,
                         SAIFERV12_KEY *key)
{
    SAIFERV12_u128 *rk_head;
    SAIFERV12_u128 *rk_tail;
    register uint32_t w1, w2;
    register uint32_t reg0, reg1, reg2, reg3;
    uint32_t s0, s1, s2, s3;

    const int r = saiferv12_set_encrypt_key(userKey, bits, key);
    ...
    memcpy(rk_head, rk_tail, SAIFERV12_BLOCK_SIZE);
    ...
    for (; rk_head < rk_tail; rk_head++, rk_tail--) {
        SAIFERV12_DEC_DIFF_BYTE(rk_head->u[0], reg0, w1, w2);
        SAIFERV12_DEC_DIFF_BYTE(rk_head->u[1], reg1, w1, w2);
        SAIFERV12_DEC_DIFF_BYTE(rk_head->u[2], reg2, w1, w2);
        SAIFERV12_DEC_DIFF_BYTE(rk_head->u[3], reg3, w1, w2);

        SAIFERV12_DIFF_WORD(reg0, reg1, reg2, reg3);
        SAIFERV12_DIFF_BYTE(reg0, reg1, reg2, reg3);
        SAIFERV12_DIFF_WORD(reg0, reg1, reg2, reg3);
        ...
        SAIFERV12_DEC_DIFF_BYTE(rk_tail->u[0], reg0, w1, w2);
        SAIFERV12_DEC_DIFF_BYTE(rk_tail->u[1], reg1, w1, w2);
        SAIFERV12_DEC_DIFF_BYTE(rk_tail->u[2], reg2, w1, w2);
        SAIFERV12_DEC_DIFF_BYTE(rk_tail->u[3], reg3, w1, w2);
...
static const SAIFERV12_u128 c1 = {{
    0x51, 0x7c, 0xc1, 0xb7, 0x27, 0x22, 0x0a, 0x94,
    0xfe, 0x13, 0xab, 0xe8, 0xfa, 0x9a, 0x6e, 0xe0
}};

static const SAIFERV12_u128 c2 = {{
    0x6d, 0xb1, 0x4a, 0xcc, 0x9e, 0x21, 0xc8, 0x20,
    0xff, 0x28, 0xb1, 0xd5, 0xef, 0x5d, 0xe2, 0xb0
}};

static const SAIFERV12_u128 c3 = {{
    0xdb, 0x92, 0x37, 0x1d, 0x21, 0x26, 0xe9, 0x70,
    0x03, 0x24, 0x97, 0x75, 0x04, 0xe8, 0xc9, 0x0e
}};

static void xor128(SAIFERV12_c128 o, const SAIFERV12_c128 x, const SAIFERV12_u128 *y)
{
    int i;

    for (i = 0; i < SAIFERV12_BLOCK_SIZE; i++)
        o[i] = x[i] ^ y->c[i];
}

static ossl_inline void rotnr(unsigned int n, SAIFERV12_u128 *o,
                              const SAIFERV12_u128 *xor, const SAIFERV12_u128 *z)
{
    ...
    for (i = 0; i < SAIFERV12_BLOCK_SIZE; i++)
        t.c[(i + bytes) % SAIFERV12_BLOCK_SIZE] = z->c[i];
    for (i = 0; i < SAIFERV12_BLOCK_SIZE; i++)
        o->c[i] = ((t.c[i] >> bits) |
                (t.c[i ? i - 1 : SAIFERV12_BLOCK_SIZE - 1] << (8 - bits))) ^
                xor->c[i];
}

static void rot19r(SAIFERV12_u128 *o, const SAIFERV12_u128 *xor, const SAIFERV12_u128 *z)
{
    rotnr(19, o, xor, z);
}

static void rot31r(SAIFERV12_u128 *o, const SAIFERV12_u128 *xor, const SAIFERV12_u128 *z)
{
    rotnr(31, o, xor, z);
}

static void rot61l(SAIFERV12_u128 *o, const SAIFERV12_u128 *xor, const SAIFERV12_u128 *z)
{
    rotnr(8 * SAIFERV12_BLOCK_SIZE - 61, o, xor, z);
}

static void rot31l(SAIFERV12_u128 *o, const SAIFERV12_u128 *xor, const SAIFERV12_u128 *z)
{
    rotnr(8 * SAIFERV12_BLOCK_SIZE - 31, o, xor, z);
}

static void rot19l(SAIFERV12_u128 *o, const SAIFERV12_u128 *xor, const SAIFERV12_u128 *z)
{
    rotnr(8 * SAIFERV12_BLOCK_SIZE - 19, o, xor, z);
}

static void sl1(SAIFERV12_u128 *o, const SAIFERV12_u128 *x, const SAIFERV12_u128 *y)
{
    unsigned int i;
    for (i = 0; i < SAIFERV12_BLOCK_SIZE; i += 4) {
...
static void sl2(SAIFERV12_c128 o, const SAIFERV12_u128 *x, const SAIFERV12_u128 *y)
{
    unsigned int i;
    for (i = 0; i < SAIFERV12_BLOCK_SIZE; i += 4) {
        o[i    ] = sb3[x->c[i	 ] ^ y->c[i    ]];
...
static void a(SAIFERV12_u128 *y, const SAIFERV12_u128 *x)
{
...
static ossl_inline void FO(SAIFERV12_u128 *o, const SAIFERV12_u128 *d,
                           const SAIFERV12_u128 *rk)
{
    SAIFERV12_u128 y;
...
static ossl_inline void FE(SAIFERV12_u128 *o, const SAIFERV12_u128 *d,
                           const SAIFERV12_u128 *rk)
{
    SAIFERV12_u128 y;
...
static void do_encrypt(unsigned char *o, const unsigned char *pin,
                       unsigned int rounds, const SAIFERV12_u128 *keys)
{
    SAIFERV12_u128 p;
    unsigned int i;
...
int saiferv12_set_encrypt_key(const unsigned char *userKey, const int bits,
                         SAIFERV12_KEY *key)
{
    const SAIFERV12_u128 *ck1, *ck2, *ck3;
    SAIFERV12_u128 kr, w0, w1, w2, w3;
    ...
    case 192:
        ...
        ck2 = &c3;
        ck3 = &c1;
        memcpy(kr.c, userKey + SAIFERV12_BLOCK_SIZE, sizeof(kr) / 2);
        memset(kr.c + SAIFERV12_BLOCK_SIZE / 2, 0, sizeof(kr) / 2);
        break;
    ...
    case 256:
        ...
        ck2 = &c1;
        ck3 = &c2;
        memcpy(kr.c, userKey + SAIFERV12_BLOCK_SIZE, sizeof(kr));
        break;
    }
...
{
    SAIFERV12_KEY ek;
    const int r = saiferv12_set_encrypt_key(userKey, bits, &ek);
